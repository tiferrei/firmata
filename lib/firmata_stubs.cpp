...
#define getObject(kind, obj) (kind *)*((kind **)Data_custom_val(obj))

void deleteSerial(value obj) {
    CAMLparam1(obj);
    Serial *serial_obj = getObject(Serial, obj);
    serial_obj->Close();
    delete serial_obj;
    CAMLreturn0;
}
...
value newSerial() {
    ...
    obj = caml_alloc_custom(&serial_custom, sizeof(Serial *), sizeof(Serial), 1);
...
value writeSerial(value obj, value list) {
    ...
    if (i > 0) serial_obj->Write(buffer, i);
