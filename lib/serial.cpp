...
#include <windows.h>
#define win32_err(s) FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, NULL, GetLastError(), 0, (s), sizeof(s), NULL)
...
int Serial::Open(const string &name) {
    ...
    if (port_fd < 0) {
        if (errno == EACCES) {
            ...
            error_msg = "Unable to open " + name + ", Object is a directory, not a serial port";
        } else if (errno == ENODEV || errno == ENXIO) {
            error_msg = "Unable to open " + name + ", Serial port hardware not installed";
        } else if (errno == ENOENT) {
            error_msg = "Unable to open " + name + ", Device name does not exist";
        } else {
    ...
    port_handle =
        CreateFile(name_createfile, GENERIC_READ | GENERIC_WRITE, 0, 0, OPEN_EXISTING, FILE_FLAG_OVERLAPPED, NULL);
...
string Serial::error_message(void) { return error_msg; }

int Serial::Is_open(void) { return port_is_open; }
...
static void macos_ports(io_iterator_t *PortIterator, vector<string> &list) {
    ...
    while ((modemService = IOIteratorNext(*PortIterator))) {
        nameCFstring =
            IORegistryEntryCreateCFProperty(modemService, CFSTR(kIOCalloutDeviceKey), kCFAllocatorDefault, 0);
        if (nameCFstring) {
